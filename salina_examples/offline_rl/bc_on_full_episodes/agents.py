...
class ActionMLPAgentFromTransformer(Agent):
    def __init__(self, env, n_layers, hidden_size,embedding_size,input_name="attn_out/x"):
        super().__init__()
        env = instantiate_class(env)
...
def transition_transformers(encoder,transformer,decoder):
    ...
    mblock=TransformerMultiBlockAgent(transformer.n_layers,encoder.embedding_size,transformer.n_heads,use_layer_norm=transformer.use_layer_norm)
    internal_action_agent=ActionMLPAgentFromTransformer(decoder.env,decoder.n_layers, decoder.hidden_size,encoder.embedding_size)
