...
class TransitionEncoder(Agent):
    ...
    def __init__(self,env,n_layers,hidden_size,embedding_size,max_episode_steps,output_name="attn_in/x"):
        super().__init__()
    ...
    def forward(self,t=None,**kwargs):
        if not t is None:
            if t==0:
                ...
                t_s=_timestep(self.get(("env/timestep",t)))
                assert t_s.max().item()<self.n_t_embeddings,"Episode too long coparing to time embeddings: "+str(t_s.max().item())
...
def action_transformer(encoder,transformer,decoder):
    ...
    mblock=TransformerMultiBlockAgent(transformer.n_layers,encoder.embedding_size,transformer.n_heads,n_steps=transformer.n_steps,prefix="action_attn_",use_layer_norm=transformer.use_layer_norm)
    internal_action_agent=ActionAgent(decoder.env,decoder.n_layers, decoder.hidden_size,encoder.embedding_size)
...
def critic_transformer(encoder,transformer,decoder):
    ...
    mblock=TransformerMultiBlockAgent(transformer.n_layers,encoder.embedding_size,transformer.n_heads,n_steps=transformer.n_steps,prefix="critic_attn_",use_layer_norm=transformer.use_layer_norm)
    internal_critic_agent=CriticAgent(decoder.env,decoder.n_layers, decoder.hidden_size,encoder.embedding_size)
