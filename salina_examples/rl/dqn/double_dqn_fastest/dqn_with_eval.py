...
def run_dqn(q_agent, logger, cfg):
    ...
    q_evaluation_agent = copy.deepcopy(q_agent)
    ...
    # For EVALUATION
    remote_evaluation_agent = RemoteAgent(
        TemporalAgent(
            Agents(env_evaluation_agent, q_evaluation_agent),
        ),
        num_processes=cfg.algorithm.evaluation.n_processes,
    )

    remote_evaluation_agent.seed(cfg.algorithm.evaluation.env_seed)
    ...
    evaluation_workspace = remote_evaluation_agent(
    ...
    epsilon_by_timesteps = lambda epoch: cfg.algorithm.epsilon_final + (
        cfg.algorithm.epsilon_start - cfg.algorithm.epsilon_final
    ) * math.exp(-1.0 * epoch / cfg.algorithm.epsilon_exploration_decay_per_timestep)
    ...
    for epoch in range(cfg.algorithm.max_epoch):
        epsilon = epsilon_by_timesteps(n_t)
        logger.add_scalar("monitor/epsilon", epsilon, epoch)

        if not remote_evaluation_agent.is_running():
            ...
            q_evaluation_agent.load_state_dict(_state_dict(q_agent, "cpu"))
            evaluation_workspace.copy_n_last_steps(1)

            remote_evaluation_agent.asynchronous_forward_(
                evaluation_workspace,
                epsilon=0.0,
                t=1,
            )

        if not remote_train_agent.is_running():
            ...
            workspace = remote_train_agent.asynchronous_forward_(
                workspace,
                t=cfg.algorithm.overlapping_timesteps,
                epsilon=epsilon,
            )
            n_t += (
                cfg.algorithm.n_timesteps - cfg.algorithm.overlapping_timesteps
            ) * cfg.n_envs
            n_acquired += 1
            logger.add_scalar(
                "monitor/n_trajectories_per_epoch", n_acquired / (epoch + 1), epoch
            )
            logger.add_scalar("monitor/n_interactions", n_t, epoch)
