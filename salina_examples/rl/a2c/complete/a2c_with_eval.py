...
def run_a2c(a2c_agent, logger, cfg):
    ...
    a2c_evaluation = copy.deepcopy(a2c_agent)
    ...
    t_evaluation_agent = TemporalAgent(Agents(env_evaluation_agent, a2c_evaluation))
    ...
    evaluation_workspace = evaluation_agent(
        evaluation_workspace, replay=False, stochastic=False
    )
    ...
    for epoch in range(cfg.algorithm.max_epochs):
        if not evaluation_agent.is_running():
            ...
            a2c_evaluation.load_state_dict(_state_dict(a2c_agent_cuda, "cpu"))
            evaluation_workspace.copy_time(
                from_time=-1,
                to_time=0,
            )
            evaluation_agent.asynchronous_forward_(
                evaluation_workspace, t=1, replay=False, stochastic=False
            )
