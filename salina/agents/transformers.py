...
class XFormerTransformerBlockAgent(Agent):
    def __init__(self,embedding_size,n_heads,n_steps=None,input_name="attn_in/x",output_name="attn_out/x",use_layer_norm=False):
        super().__init__()
        from xformers.components.attention import ScaledDotProduct
        self.n_steps=n_steps
        self.multiheadattention=ScaledDotProduct()
        self.input_name=input_name
        self.output_name=output_name
        if use_layer_norm:
            self.ln1=nn.LayerNorm(embedding_size)
...
class XFormerTransformerMultiBlockAgent(Agents):
    def __init__(self,n_layers,embedding_size,n_heads,n_steps=None,prefix="attn_",use_layer_norm=False):
        agents=[]
        for k in range(n_layers):
            in_prefix=prefix+str(k+1)
            ...
            if k==0:
                in_prefix=prefix+"in"
            agents.append(XFormerTransformerBlockAgent(embedding_size,n_heads,n_steps,in_prefix+"/x",out_prefix+"/x",use_layer_norm=use_layer_norm))
        super().__init__(*agents)
...
if __name__=="__main__":
    ...
    print(y2)
    print(((y1-y2)**2).lt(0.0000001).all())
