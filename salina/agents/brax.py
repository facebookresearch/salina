...
class BraxAgent(TAgent):
    def __init__(
        self, n_envs, env_name, input="action", output="env/",  **args
    ):
        super().__init__()
        self.args=args
    ...
    def forward(self, t=0, **args):
        ...
        if t == 0 or self.timestep==0:
            ...
            self.cumulated_reward=torch.zeros(self.n_envs,device=self._device).float()
            ret = {
                "env_obs":o,
                "done": torch.tensor([False],device=self._device).repeat(self.n_envs),
                "initial_state": torch.tensor([True],device=self._device).repeat(self.n_envs),
                "reward": torch.zeros(self.n_envs,device=self._device).float(),
                "timestep": torch.zeros(self.n_envs,device=self._device).long(),
            ...
            ret = {
                "env_obs":obs,
                "done": done,
                "initial_state": torch.tensor([False],device=self._device).repeat(self.n_envs),
                "reward": rewards.float(),
                "timestep": torch.tensor([self.timestep],device=self._device).repeat(self.n_envs).long(),
                "cumulated_reward": self.cumulated_reward,
            }
            if done.any():
                assert done.all()
