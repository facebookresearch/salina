...
def ActionAgent(input_dimension,output_dimension, hidden_size, start_steps):
    """
    Baseline model: a single policy that is re-used and fine-tuned over the task sequences.
    """
    return CRLAgents(Action(input_dimension,output_dimension, hidden_size, start_steps, input_name = "env/env_obs"))

def MultiActionAgent(input_dimension,output_dimension, hidden_size, start_steps):
    """
    Fine-tune and clone: the model is saved when the task is ended, and duplicated to be fine-tuned on the next task.
    """
    return CRLAgents(MultiAction(input_dimension,output_dimension, hidden_size, start_steps, input_name = "env/env_obs"))
...
def EWCActionAgent(input_dimension,output_dimension, hidden_size, fisher_coeff, start_steps):
    """
    EWC regularizer added on top of the ActionAgent model. (see )
    """
    return CRLAgents(EWCAction(input_dimension,output_dimension, hidden_size, fisher_coeff, start_steps, input_name = "env/env_obs"))

def L2ActionAgent(input_dimension,output_dimension, hidden_size, l2_coeff, start_steps):
    """
    L2 regularizer added on top of the ActionAgent model.
    """
    return CRLAgents(L2Action(input_dimension,output_dimension, hidden_size, l2_coeff, start_steps, input_name = "env/env_obs"))
...
class Action(CRLAgent):
    def __init__(self, input_dimension,output_dimension, hidden_size, start_steps = 0, input_name = "env/env_obs"):
        super().__init__()
...
class EWCAction(Action):
    def __init__(self, input_dimension,output_dimension, hidden_size, fisher_coeff, start_steps = 0, input_name = "env/env_obs"):
        super().__init__(input_dimension,output_dimension, hidden_size, start_steps, input_name)
